...
{
    public class WebSearchRag
    {
        /****** DEV NOTES *****
         *  Currently the web scraping is handled by the web scraper. I think it would be much simpler if the
         *  web scraping was done by the parserclass. 
         */
        public static async Task Run()
        {
            //Model Location
            var fullModelName = "Llama-3.2-1B-Instruct-Q4_0.gguf";
            var modelPath = @$"C:\Projects\Models\{fullModelName}";
   

            //Local document file location and embedding storage location
            string StorageFolder = @$"C:\Projects\SourceData\WebSearchRAG_{fullModelName}"; //folder to store the embeddings
            bool overWrite = false; //Always overwrite embeddings. 

            //WebscrapingParameters
            var startUrl = "https://en.wikipedia.org/wiki/Aluminium_alloy"; 
            var depth = 0;//Scrape only the webpage provided and links up to depth. 
            var minWordLength = 5; //Specify the minimum number of words in a block that should be scraped.
            var checkSentences = true;
            var explodeParagraphs = true;
            ...
            //Define Context for use in the 
            var context = new LLamaContext(model, modelparams);

            //var chunker = new Chunker(context, 256, 10); //maxChunkSize = 256 tokens , chunk overlap = 10
            var embedder = new LLamaEmbedder(model, modelparams);



            //Create instance of embeddingHandler to create and store embeddings
            var storageHandler = new StorageHandler(StorageFolder, embedder);

            //initialiaze web scraper and generate a list of Websites and Scraped Content
            var ScrapedSites = await WebScraper.CreateAsync(startUrl, depth);
            ...
            foreach (var Site in ScrapedSites.Websites)
            {
                var source = new SourceFile()
                {
                    SourceType = ".html",
                    Url = Site.Url,
                    FileName = Site.Title, //DEV NOTE - This should probably change
                    TextContentList = await ScrapedSites.ExtractParagraphsAsync(Site,false)
                };
                await storageHandler.SaveSource(source, overWrite);
             
            }

            //Load the chunks that have been created
            DataTable chunks = await storageHandler.GetAllChunks();


            //Create Interactive Executor needed for chat
            ...
            while (true)
            {
                ...
                var n_top_matches = 5;
                var topMatches = chunks.AsEnumerable()
        ...
        private static DataTable ComputeSimilarityScores(DataTable embeddings, List<float[]> queryEmbeddings)
        {
            ...
            var augmentedTable = embeddings.Clone();
            ...
            foreach (DataRow row in embeddings.Rows)
            {
                ...
                var similarityScore = (float)(embeddingVector != null
                ...
                var newRow = augmentedTable.NewRow();
